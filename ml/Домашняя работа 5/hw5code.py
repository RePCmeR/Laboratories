...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    y = target_vector[order].astype(np.int64)

    diff_mask = np.diff(x) > 0
    if not diff_mask.any():
        return np.array([]), np.array([]), None, None

    left_cnt = np.arange(1, n)[diff_mask]
    left_sum = np.cumsum(y)[left_cnt - 1]

    p_left = left_sum / left_cnt
    p_right = (np.cumsum(y)[-1] - left_sum) / (n - left_cnt)
    H_left = 2 * p_left * (1 - p_left)
    H_right = 2 * p_right * (1 - p_right)

    ginis = -(left_cnt / n) * H_left - ((n - left_cnt) / n) * H_right
    thresholds = (x[left_cnt - 1] + x[left_cnt]) / 2

    ties = np.where(ginis == ginis[np.argmax(ginis)])[0]
    best_tie = ties[np.argmin(thresholds[ties])]
    threshold_best = thresholds[best_tie]
    gini_best = ginis[best_tie]
    ...
    * ограничения max_depth, min_samples_split, min_samples_leaf (как в sklearn по смыслу)
    ...
    def _fit_node(self, sub_X, sub_y, node, depth=0):
        ...
        for feature in range(sub_X.shape[1]):
            ...
            if feature_type == "real":
                ...
                for key, current_count in counts.items():
                    if key in clicks:
                        ...
                        current_click = 0
                    ratio[key] = current_click / current_count if current_count != 0 else 0.0
            ...
            valid = np.ones(len(ginis), dtype=bool)
            if self._min_samples_leaf is not None:
                valid &= (left_cnt >= self._min_samples_leaf) & ((n - left_cnt) >= self._min_samples_leaf)
            if not np.any(valid):
                continue

            threshold = thresholds[valid][np.argmax(ginis[valid])]
            gini = ginis[valid][np.argmax(ginis[valid])]
